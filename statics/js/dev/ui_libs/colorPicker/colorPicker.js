 ...
 * $Id: colorPicker.js 12599 2012-06-25 03:10:52Z chris.chencq $			:
 */
;(function ( $, window, document, undefined ) {
    ...
    Plugin.prototype.init = function () {
    	this.container = $('<div class="colorPicker" style="display:none" />').html(htmlGeneralPanel + htmlSeniorPanel).appendTo('body');
    	var me = this;
		...
		element.on(options.event,function() {
			...
			container.css( {position:position, zIndex:zIndex, left:left + 'px', top:top + 'px'} ).show();
		});
		...
		//点击切换高级面板
		container.find('.J_set_general').on('click',function() {
			$('.wind_color_general').hide();
			$('.wind_color_senior').show();
		});
		
		//点击切换到普通模式
		container.find('.J_set_senior').on('click',function() {
        ...
        container.find('.J_default_color').on('click',function(e) {
            e.preventDefault();
            console.log(me.options.default_color)
