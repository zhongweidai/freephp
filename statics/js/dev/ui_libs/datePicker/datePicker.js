 ...
 * $Id: datePicker.js 12522 2012-06-23 03:33:08Z chris.chencq $			:
 */
;(function($, window, document, undefined) {
	...
	function format(date, fmt, lang) {
		var d = date.getDate(), D = date.getDay(), m = date.getMonth(), y = date.getFullYear(), flags = {
			d : d,
			dd : zeropad(d),
	...
	Plugin.prototype.init = function() {
		...
		if(!root.length) {
			...
			if(options.time) {
				...
				$('<div class="caltime"><input type="number" min="0" max="23" size="2">点<input type="number" size="2" min="1" max="59">分<button type="button" name="submit">确认</button></div>').appendTo(body);
