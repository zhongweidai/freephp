...
;(function ( $, window, undefined ) {
	...
	WindEditor.plugin(pluginName,function() {
		...
		var plugin_icon = $('<div class="wind_icon"><span class="'+ pluginName +'" title="插入表情"></span></div>').appendTo( big_icon_box );
		...
		plugin_icon.on('click',function() {
			...
			if(!$.contains(document.body,dialog[0]) ) {
				...
				//填充表情数据
				$.getJSON(emotionUrl,function(data) {
					emotionData = data;
					...
					$.each(data.data,function(key,obj) {
						...
						//点击表情插入
						emotion_box.on('click','.edit_menu_cont li',function(e) {
							e.preventDefault();
