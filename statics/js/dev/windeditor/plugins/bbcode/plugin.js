...
;(function ( $, window, undefined ) {
	...
	function ubb2html(s) {
	    ...
	    rep(/\[(\/?)(sup|sub)\]/ig, '<$1$2>');
	    rep(/\[color\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig, '<span style="color:$1">');
	    rep(/\[size\s*=\s*(\d+?)\s*\]/ig, '<font size="$1">');
	    rep(/\[font\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig, '<span style="font-family:$1">');
	    rep(/\[\/(color|size|font)\]/ig, '</span>');
	    ...
	    rep(/\[img\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/img\]/ig, '<img src="$1" alt="" />');
	    ...
	    rep(/\[td\s*(?:=\s*(\d{1,2})\s*,\s*(\d{1,2})\s*(?:,\s*(\d{1,4}%?))?)?\s*\]/ig, function (all, col, row, w) {
		...
		//s = '<div'+style+'>' + rep(/\r?\n/g, '</div><div'+style+'>') +'</div>';
		//rep(/<div>\s*<\/div>/ig,'<div'+style+'>&nbsp;</div>');
		rep(/\r?\n/ig,"<br />");
		rep(/\r/ig,"<br />");
