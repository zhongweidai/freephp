...
;(function ( $, window, undefined ) {
	...
	WindEditor.plugin(pluginName,function() {
		...
		var plugin_icon = $('<div class="wind_icon"><span class="'+ pluginName +'" title="at"></span></div>').appendTo( big_icon_box );
		//依赖页面中PHP输出的请求地址
		var atUrl = window.atUrl;
		//弹窗的地址，弹窗中有一些后端输出，所以需要使用ajax去加载
		var atDialogUrl = window.atDialogUrl;
		//用户的空间地址
		var atUserSpace = window.atUserSpace;
		var atDialogDom;
		...
		*/
		...
		function dialogAfterShow() {
			...
			$(_self.editorDoc).find('a').each(function() {
				if(this.href.indexOf(atUserSpace) !== -1) {
		...
		function dialogEventInit() {
			...
			atDialogDom.find('.edit_menu_btn').on('click',function(e) {
				...
				friend_selected.find('.J_friend_name').each(function() {
					...
					arr.push('<span><a target="_blank" class="J_at_link" data-role="at" href="'+ atUserSpace + uid +'">@' + username + '<a/></span>');
				});
		...
		*/
		$(editorDoc).on({
			keydown:function(e) {
				...
				//是否开启输入法
				var ime_mode = e.shiftKey && (e.keyCode === 229 || e.keyCode === 197);
				var isAt = e.shiftKey && e.keyCode === 50;
				if(isAt) {
					e.preventDefault();
					var id = "tmp" + (+new Date());
		...
		//显示最近@过的朋友
		function showRecentFriendData(span) {
			if(recentFriendData) {
				showResultOptions(recentFriendData,span);
			}else{
				$.getJSON(atUrl,function(data) {
					if(data.state === 'success') {
						recentFriendData = data;//缓存起来
						showResultOptions(data,span);
					}else {
						$.error(data.message);
					}
				});
		...
		//显示@朋友结果
		function showFriendData(username,span) {
			if(friendData) {
				var data = filterData(username,friendData);//过滤
				showResultOptions(data,span);
			}else{
				//如果没有缓存数据则请求结果
				$.getJSON(atUrl,{username:username},function(data) {
					if(data.state === 'success') {
						friendData = data;//缓存起来
						var newData = filterData(username,friendData);
						showResultOptions(newData,span);
					}else {
						$.error(data.message);
					}
				});
		...
		function choose() {
			...
			if(current.length) {
				...
				var at = $('<span><a target="_blank" class="J_at_link" href="'+ atUserSpace + uid + '" data-role="at" data-username="'+ username +'">@'+ username +'</a>&nbsp;</span>');
				span.replaceWith(at);
				var range = _self.getRange();
				if(range.moveToElementText) {
					range.moveToElementText(nativeNode);
				}else {
					var node = at[0].lastChild;
		...
		$(form).on('submit',function(e) {
			...
			$(_self.editorDoc).find('a').each(function() {
				if(this.href.indexOf(atUserSpace) !== -1) {
					var id = this.href.replace(atUserSpace,'');
					arr.push('<input type="hidden" name="remind[]" value="'+ id +'"/>');
				}
			});
