...
;(function ( $, window, undefined ) {
		...
		dialog = $('<div class="edit_menu" style="display:none;">\
					<div class="edit_menu_hide">\
						...
						<div class="edit_menu_cont">\
							<ul>\
								<li>允许设置最高售价50，限制最高收入2000</li>\
								<li>售价：<input name="price" type="text" size="2" class="input length_1 mr5 J_price" value="2">\
								<select name="unt" class="mr5 J_unit">'+ creditSelect +'</select>\
	...
	WindEditor.plugin(pluginName,function() {
		...
		var plugin_icon = $('<div class="wind_icon" unselectable="on"><span class="'+ pluginName +'" title="插入出售内容"></span></div>').insertAfter( icon_ul.find('span.insertBlockquote').parent() );
		...
		var style = "<style>\
			.content_sell {border:1px dashed #95c376;padding:10px 40px;margin:5px 0;background:#f8fff3;}.content_sell h5 {font-size:12px;color:#669933;margin-bottom:5px;}</style>";
		$(head).append(style);
		...
		dialog.find('.edit_menu_btn').on('click',function(e) {
			...
			var val = '<div class="content_sell" data-price="'+ price +'" data-unit="'+ unit +'"><h5>本帖出售的内容</h5>'+ textarea.val() +'</div>';
			textarea.val('');
		...
		function wysiwyg() {
			...
			html = html.replace(reg,function(all, $1, $2,$3) {
				return '<div data-price="'+ $1 +'" data-unit="'+ $2 +'" class="content_sell"><h5>本帖出售的内容</h5>'+ $3 +'</div>';
			});
