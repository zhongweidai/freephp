...
;(function(){
			...
			if(ajaxTempError(data)) {
				Wind.dialog.html(data, {
					...
					callback	: function(){
						...
						buttonStatus(domain, check_domain);
						...
						var temp_list = $('#J_temp_list'),													//模板列表
							...
							li_height = temp_list.children().outerHeight('includeMargin'),
						...
						function tempMove(page_index){
							...
							//当前的最后一个项索引
							var li_index = 8*(page_index+1) - 1;
							
							//判断此项style是否存在
							if(!temp_list.children(':eq('+ li_index +')').children().attr('style')) {
								...
								$.each(li_lt, function(i, o){
									...
									if(!child.attr('style')) {
										child.attr('style', child.data('style'));
									}
								});
							}
						...
						//上传控件切换
						var space_bg_preview = $('#J_space_bg_preview'),
	...
	$('a.J_space_follow').on('click', function(e){
      ...
      $.getJSON(url, {uid : $this.data('uid')}, function(data){
         if(data.state == 'success') {
            if(role == 'follow') {
