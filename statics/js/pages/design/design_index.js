...
*/
	...
	mod_wrap.on('click', 'a.J_layout_edit', function(e){
		...
		//if(!$this.data('load')) {
			//还没加载或已提交
			$.post(LAYOUT_EDID_TITLE, {name : layout_id, pageid : pageid}, function(data){
	...
	mod_wrap.on('click', 'a.J_module_edit', function(e){
		...
		$.post(MODULE_INFO, {
			...
			if(data.state == 'success') {
				design_name.text('挂件管理['+ moduleid + '][' + data.data + ']');
			} else{
				//global.js
				resultTip({
					error : true,
		...
		//design_name.text('挂件管理-'+ moduleid);
	...
	design_del.on('click', function(e){
		...
		if(role == 'layout') {
			...
			if(confirm('您确定要删除本挂件吗？删除后将不可恢复！')) {
				...
				$.post(MODULE_BOX_DEL, {moduleid: moduleid, pageid :pageid}, function(data){
					//global.js
					ajaxMaskRemove();
...
*/
				...
				$('form.J_design_module_form').ajaxForm({
					...
					success : function(data, statusText, xhr, $form){
						if(data.state == 'success') {
							...
							//写入挂件html
							$.post(MODULE_BOX_UPDATE, {moduleid : moduleid, pageid :pageid}, function(data){
								if(ajaxTempError(data)) {
										ajaxMaskRemove();
										popHide();
										mudule_box.html('<a id="'+ moduleid +'" class="J_module_edit" style="display:none;" href="#">编辑</a><div class="design_list_1 J_module_con_list" id="module_'+ moduleid +'">'+ data +'</div>').attr('id', 'J_mod_'+ moduleid);
								}
							});
		...
		$.post(layout_edit_nav.children('.current').children().attr('href'), {
			...
			if(ajaxTempError(data)) {
				...
				if(updatemod && mudule_box.children().length > 1) {
					updateModuleList(moduleid, mudule_box);
				}
			}
		});
	...
	//更新挂件列表内容
	function updateModuleList(moduleid, mudule_box){
		//有列表内容
		...
		for(i=0;i<mod_wrap.length;i++){
			...
			for(k=0;k<box.length;k++) {
				...
				if(_id) {
					...
					var el_module = ' {pc:widgets action="detail" widgetid="'+widgetid+'" pageid="$pageid"}{/pc}';
